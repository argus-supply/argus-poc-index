...
from sync.core import ROOT, canonical, digest, load_policy, read_snapshot, apply_result, build_snapshot, semantic
from sync.adapters import AdapterResult
from sync.dependency import consume_intel, read_archive, read_manifest
from sync.gitstore import GitStore, Ledger
from sync.http import BudgetExceeded, Http
from sync.run import run
from tests.remotes import init_bare_remote
...
def codeload_archive(files, sha='a' * 40, *, members=None):
    """Build the exact fixed-SHA archive shape emitted by codeload fixtures."""
    output = io.BytesIO()
    root = 'argus-intel-data-' + sha
    with tarfile.open(fileobj=output, mode='w:gz') as archive:
        directory = tarfile.TarInfo(root)
        directory.type = tarfile.DIRTYPE
        archive.addfile(directory)
        for path, body in (members if members is not None else files.items()):
            item = tarfile.TarInfo(root + '/' + path)
            item.size = len(body)
            item.mode = 0o644
...
class DependencyTests(unittest.TestCase):
    ...
    def test_dependency_coverage_requires_every_fixed_source_watermark_and_no_gaps(self):
        for override in ({}, {'cve': {'status': 'partial'}}, {'ghsa': {'completed_watermark': None}},
            ...
            with self.subTest(override=override):
                ...
                class Remote:
                    def get_bytes(self, url):
                        return codeload_archive(files)
                dependency, cache, persisted = consume_intel(Remote(), 'a' * 40, None, {})
                self.assertEqual(dependency['coverage_complete'], not bool(override))
                self.assertEqual(cache['commit_sha'], 'a' * 40)
    ...
    def test_fresh_dependency_uses_one_fixed_sha_archive_request(self):
        files = self.intel_files()
        calls = []
        class Remote:
            def get_bytes(self, url):
                calls.append(url)
                return codeload_archive(files)
        dependency, _, _ = consume_intel(Remote(), 'a' * 40)
        self.assertTrue(dependency['coverage_complete'])
        self.assertEqual(calls, [
            'https://codeload.github.com/argus-supply/argus-intel-data/tar.gz/' + 'a' * 40])

    def test_archive_stream_rejects_member_limit_links_and_path_escape(self):
        files = self.intel_files()
        root = 'argus-intel-data-' + 'a' * 40

        def malformed(name, kind=tarfile.REGTYPE):
            output = io.BytesIO()
            with tarfile.open(fileobj=output, mode='w:gz') as archive:
                directory = tarfile.TarInfo(root)
                directory.type = tarfile.DIRTYPE
                archive.addfile(directory)
                manifest = tarfile.TarInfo(root + '/manifest.json')
                manifest.size = len(files['manifest.json'])
                archive.addfile(manifest, io.BytesIO(files['manifest.json']))
                item = tarfile.TarInfo(name)
                item.type = kind
                item.linkname = root + '/manifest.json'
        ...
        with patch('sync.dependency.ARCHIVE_MAX_MEMBERS', 2), \
                self.assertRaisesRegex(ValueError, 'too many members'):
            read_archive(Remote(malformed(root + '/extra')), 'argus-supply/argus-intel-data', 'a' * 40)
        for body, message in (
            (malformed(root + '/link', tarfile.SYMTYPE), 'non-regular'),
            (malformed(root + '/../escape'), 'unsafe')):
            with self.subTest(message=message), self.assertRaisesRegex(ValueError, message):
                read_archive(Remote(body), 'argus-supply/argus-intel-data', 'a' * 40)

    def test_old_dependency_cache_is_not_reused_or_persisted(self):
        files = self.intel_files()
        old = {'repository': 'argus-supply/argus-intel-data', 'commit_sha': 'a' * 40,
            'manifest_sha256': digest(files['manifest.json']),
            'shards': [{'path': 'state/intel/legacy.jsonl'}]}
        ...
        class Remote:
            ...
            def get_bytes(self, url):
                calls.append(url)
                if not url.endswith('/' + 'b' * 40):
                    raise AssertionError('dependency did not use the resolved immutable SHA')
                return codeload_archive(files, 'b' * 40)
        dependency, upgraded, projections = consume_intel(
            Remote(), None, {'dependency_cache': old}, {'state/intel/legacy.jsonl': b'legacy'})
        self.assertEqual(old, original)
        self.assertTrue(dependency['coverage_complete'])
        self.assertEqual(dependency['commit_sha'], 'b' * 40)
        self.assertEqual(upgraded['commit_sha'], 'b' * 40)
        self.assertEqual(projections, {})
        count = len(calls)
        repeated, _, _ = consume_intel(Remote(), 'b' * 40, {'dependency_cache': old},
                                       {'state/intel/legacy.jsonl': b'legacy'})
        self.assertEqual(repeated, dependency)
        self.assertGreater(len(calls), count)

    def test_dependency_rejects_manifest_repository_mismatch(self):
        good = self.intel_files()
        body = canonical({**json.loads(good['manifest.json']), 'repository': 'wrong/repo'})
        class Remote:
            def get_bytes(self, url):
                return codeload_archive({'manifest.json': body})
        with self.assertRaisesRegex(ValueError, 'archive repository mismatch'):
            consume_intel(Remote(), 'a' * 40)
    ...
    def test_continuations_restore_references_and_unchanged_intel_only_resolves_head(self):
        ...
        count = len(calls)
        second, next_cache, _ = consume_intel(Remote(), None,
            {'intel_dependency_state': cache}, cached_files)
        self.assertEqual(second['active_ids'], first['active_ids'])
    ...
    def test_checkpoint_deltas_apply_upserts_and_deletes_with_fixed_shard_hashes(self):
        policy = load_policy(ROOT / 'policy.json')
        row = json.loads((ROOT / 'fixtures/source-records.json').read_text())[0]
        old = copy.deepcopy(row)
        old.update(record_id='ghsa/GHSA-old', native_id='GHSA-old', aliases=['GHSA-old'])
        changed = copy.deepcopy(row)
        changed.update(record_id='ghsa/GHSA-current', native_id='GHSA-current',
                       aliases=['CVE-2026-99999'], title='Old title')
        records, events, sources = {}, {}, {}
        apply_result(records, events, sources, 'ghsa',
            AdapterResult(records=[old, changed], status='ok', completed_watermark='watermark'),
            '2026-09-09T00:00:00Z', policy)
        # Dependency completeness belongs to the manifest's fixed source state,
        # independently of which sources happen to own records in this fixture.
        sources = {name: {'status': 'ok', 'completed_watermark': 'watermark',
            'continuation': None, 'coverage_gaps': [], 'errors': []}
            for name in ('cve', 'ghsa', 'kev')}
        first, first_manifest = build_snapshot('argus-supply/argus-intel-data', records, events,
            sources, policy, '2026-09-09T00:00:00Z', 'fixture')
        records.pop(old['record_id'])
        records[changed['record_id']]['title'] = 'Current title'
        records[changed['record_id']]['content_hash'] = digest(canonical(
            semantic(records[changed['record_id']])))
        second, _ = build_snapshot('argus-supply/argus-intel-data', records, events, sources,
            policy, '2026-09-10T00:00:00Z', 'fixture', first_manifest,
            previous_files=first, run_id='intel-delta')

        class Remote:
            def __init__(self, corrupt=None, track=None):
                self.corrupt, self.track = corrupt, track

            def get_bytes(self, url):
                if url.startswith('https://codeload.github.com/'):
                    sha = url.rsplit('/', 1)[-1]
                    selected = dict(second if sha == 'b' * 40 else first)
                    if self.corrupt:
                        selected[self.corrupt] += b' '
                    return codeload_archive(selected, sha)
                sha, path = url.split('argus-intel-data/', 1)[1].split('/', 1)
                if self.track is not None:
                    self.track.append((sha, path))
                files = second if sha == 'b' * 40 else first
                body = files[path]
                return body + b' ' if path == self.corrupt else body

        dependency, cache, persisted = consume_intel(Remote(), 'b' * 40)
        self.assertEqual([item['record_id'] for item in dependency['records']],
                         ['ghsa/GHSA-current'])
        self.assertEqual(dependency['records'][0]['title'], 'Current title')
        self.assertEqual(cache['commit_sha'], 'b' * 40)
        self.assertEqual(persisted, {})
        record_path = next(item['path'] for item in json.loads(second['manifest.json'])['shards']
                           if item['kind'] == 'records' and item['operation'] != 'delete')
        with self.assertRaisesRegex(ValueError, 'record shard (size )?mismatch'):
            consume_intel(Remote(record_path), 'b' * 40)

        # A normal next revision verifies both immutable manifests, then reads
        # only newly referenced record deltas--never the checkpoint shards.
        first_dependency, first_state, _ = consume_intel(Remote(), 'a' * 40)
        calls = []
        incremental, next_state, _ = consume_intel(Remote(track=calls),
            'b' * 40, {'intel_dependency_state': first_state})
        self.assertEqual(incremental['mode'], 'incremental')
        self.assertEqual(incremental['records'][0]['title'], 'Current title')
        data_paths = [path for _, path in calls if path != 'manifest.json']
        self.assertTrue(data_paths)
        self.assertTrue(all(path.startswith('deltas/') for path in data_paths))
        self.assertFalse(any(path.startswith('checkpoints/') for path in data_paths))
        self.assertEqual(next_state['commit_sha'], 'b' * 40)
...
class PocDependencyHttp(Http):
    ...
    def get_bytes(self, url, headers=None):
        ...
        elif url.startswith('https://codeload.github.com/'):
            result = codeload_archive(self.files, self.sha)
        elif '/' + self.sha + '/' in url:
...
class PocDependencyRunnerTests(unittest.TestCase):
    ...
    def test_partial_intel_keeps_poc_sources_ok_but_manifest_health_and_gate_partial(self):
        ...
        self.assertNotIn('dependency_cache', manifest)
        self.assertFalse(any(path.startswith('state/intel/') for path in data))
        self.assertEqual(manifest['dependencies'], result['dependencies'])
        self.assertEqual(manifest['dependencies'][0]['manifest_sha256'],
                         result['dependency_coverage']['argus-supply/argus-intel-data']['manifest_sha256'])
