...
from sync.core import (ROOT, apply_result, build_snapshot, canonical, digest,
    event_types, load_policy, read_snapshot, shard_rows, validate)
from sync.gitstore import GitStore, Ledger, ParentMoved
from sync.http import BudgetExceeded, FetchError, Http, Redirects, allowed
...
class CoreTests(unittest.TestCase):
    ...
    def test_collection_revision_fields_do_not_change_content_or_emit_events(self):
        row = copy.deepcopy(self.row)
        row['upstream_revision'] = 'old-revision'
        row['source_commit'] = 'a' * 40
        row['provenance'][0]['url'] = 'https://example.com/blob/' + 'a' * 40
        row['mapping_history'] = {'CVE-2026-12345': {'reason': 'mapped',
            'source_revision': 'a' * 40, 'source_index_url': 'https://example.com/a'}}
        self.apply(row)
        original = copy.deepcopy(self.records)
        event_ids = set(self.events)
        row['upstream_revision'] = 'new-revision'
        row['source_commit'] = 'b' * 40
        row['provenance'][0]['url'] = 'https://example.com/blob/' + 'b' * 40
        row['mapping_history']['CVE-2026-12345']['source_revision'] = 'b' * 40
        row['mapping_history']['CVE-2026-12345']['source_index_url'] = 'https://example.com/b'
        self.apply(row)
        self.assertEqual(self.records, original)
        self.assertEqual(set(self.events), event_ids)

    def test_checkpoint_migrates_legacy_content_hash_without_new_event(self):
        self.apply()
        event_ids = set(self.events)
        self.records[self.row['record_id']]['content_hash'] = '0' * 64
        files, _ = self.snapshot()
        restored, restored_events, _, _ = read_snapshot(files)
        self.assertNotEqual(restored[self.row['record_id']]['content_hash'], '0' * 64)
        self.assertEqual(set(restored_events), event_ids)
    ...
    def test_a07_shards_split_stably_and_have_exact_hashes(self):
        ...
        path = next(p for p in files if '/records/' in p)
        files[path] += b' '
        with self.assertRaisesRegex(ValueError, 'checksum'):
            read_snapshot(files)

    def test_record_shards_are_source_partitioned_and_capped_at_64_kib(self):
        rows = [dict(self.row, record_id=f'ghsa/{index}', title='x' * 1000)
                for index in range(200)]
        shards = shard_rows(rows, 'records', self.policy)
        self.assertTrue(all(path.startswith('records/ghsa/') for path in shards))
        self.assertTrue(all(len(data) <= 64 * 1024 for data in shards.values()))

    def test_checkpoint_delta_round_trip_uses_immutable_run_batches(self):
        self.apply()
        first_files, first_manifest = self.snapshot()
        changed = dict(self.row, status='withdrawn')
        apply_result(self.records, self.events, self.sources, 'ghsa',
            AdapterResult(records=[changed], status='ok', revision='b' * 40),
            '2026-09-10T00:00:00Z', self.policy)
        second_files, second_manifest = build_snapshot('argus-supply/argus-intel-data',
            self.records, self.events, self.sources, self.policy, '2026-09-10T00:00:00Z',
            'test', first_manifest, previous_files=first_files)
        delta = second_manifest['storage']['deltas'][0]
        self.assertTrue(any('/records/upserts/ghsa/' in path for path in delta['shards']))
        self.assertTrue(any('/events/' in path for path in delta['shards']))
        for path in first_manifest['storage']['checkpoint']['shards']:
            self.assertEqual(second_files[path], first_files[path])
        restored, restored_events, _, restored_manifest = read_snapshot(second_files)
        self.assertEqual(restored, self.records)
        self.assertEqual(restored_events, self.events)
        self.assertEqual(restored_manifest, second_manifest)

        del self.records[self.row['record_id']]
        third_files, third_manifest = build_snapshot('argus-supply/argus-intel-data',
            self.records, self.events, self.sources, self.policy, '2026-09-11T00:00:00Z',
            'test', second_manifest, previous_files=second_files)
        latest = third_manifest['storage']['deltas'][-1]
        delete_path = next(path for path in latest['shards'] if '/records/deletes/' in path)
        self.assertEqual(json.loads(third_files[delete_path]),
                         {'record_id': self.row['record_id']})
        self.assertEqual(read_snapshot(third_files)[0], {})

    def test_reader_accepts_manifest_without_storage_layout(self):
        self.apply()
        files, manifest = self.snapshot()
        legacy = copy.deepcopy(manifest)
        legacy.pop('storage')
        for descriptor in legacy['shards']:
            descriptor.pop('operation')
        files['manifest.json'] = canonical(legacy)
        restored, restored_events, _, loaded = read_snapshot(files)
        self.assertEqual(restored, self.records)
        self.assertEqual(restored_events, self.events)
        self.assertEqual(loaded, legacy)

    def test_manifest_rejects_incomplete_or_duplicate_storage_inventory(self):
        self.apply()
        _, manifest = self.snapshot()
        incomplete = copy.deepcopy(manifest)
        incomplete['storage']['checkpoint']['shards'].pop()
        with self.assertRaisesRegex(ValueError, 'inventory mismatch'):
            validate('manifest', incomplete)
        duplicate = copy.deepcopy(manifest)
        duplicate['shards'].append(copy.deepcopy(duplicate['shards'][0]))
        with self.assertRaisesRegex(ValueError, 'inventory mismatch'):
...
class HttpTests(unittest.TestCase):
    ...
    def test_codeload_allows_only_direct_fixed_intel_sha_and_never_cross_origin_redirects(self):
        sha = 'a' * 40
        valid = 'https://codeload.github.com/argus-supply/argus-intel-data/tar.gz/' + sha
        self.assertEqual(allowed(valid).path, '/argus-supply/argus-intel-data/tar.gz/' + sha)
        for url in (
            valid + '?download=1', valid + '#archive', valid[:-1], valid[:-40] + 'main',
            valid.replace('argus-supply', 'other'), valid.replace('argus-intel-data', 'other'),
            valid.replace('/tar.gz/', '/zip/')):
            with self.subTest(url=url), self.assertRaises(FetchError):
                allowed(url)
        import urllib.request
        request = urllib.request.Request('https://api.github.com/repos/argus-supply/argus-intel-data/tarball/' + sha)
        with self.assertRaisesRegex(FetchError, 'cross-origin'):
            Redirects().redirect_request(request, None, 302, '', {}, valid)
...
class GitTests(unittest.TestCase):
    ...
    def test_git_store_disables_background_object_maintenance(self):
        ...
        for key in ('maintenance.auto', 'receive.autogc'):
            remote = subprocess.run(['git', '-C', str(self.remote), 'config', '--get', key],
                capture_output=True, check=True)
            self.assertEqual(remote.stdout.strip(), b'false')
