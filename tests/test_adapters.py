...
def codeload_archive(files, sha):
    output = io.BytesIO()
...
class SourceContracts(unittest.TestCase):
    ...
    def test_official_incremental_intel_reconciles_only_changed_advisory_references(self):
        old = normalize_cve(cve(), REV)
        first = collect('official-references', HTTP(lambda u: self.fail('unexpected fetch')), {},
            now=NOW, policy={}, dependency=dependency([old]))
        changed = copy.deepcopy(old)
        changed['references'] = [{'url': 'https://example.org/replacement',
                                 'name': 'Replacement advisory'}]
        dep = {**dependency([changed]), 'commit_sha': OLDER, 'base_commit_sha': REV,
               'mode': 'incremental', 'active_ids': old['aliases'],
               'deleted_record_ids': []}
        result = collect('official-references', HTTP(lambda u: self.fail('unexpected fetch')),
            {'records': first.records, 'state': first.state}, now=NOW, policy={}, dependency=dep)
        self.assertEqual(result.status, 'ok')
        self.assertEqual(len(result.records), 2)
        self.assertEqual({row['status'] for row in result.records}, {'active', 'source_deleted'})
        active = next(row for row in result.records if row['status'] == 'active')
        self.assertEqual(active['url'], 'https://example.org/replacement')
        self.assertEqual(result.revision, OLDER)
...
class OfficialReferenceRunnerTests(unittest.TestCase):
    def test_fresh_runners_finish_once_then_do_zero_reference_units_and_data_rewrites(self):
        ...
        class PublicFixtureHttp(Http):
            ...
            def get_bytes(self, url, headers=None):
                self._before()
                prefix = 'https://raw.githubusercontent.com/argus-supply/argus-intel-data/'
                if url == ('https://codeload.github.com/argus-supply/argus-intel-data/tar.gz/'
                           + self.dependency_sha):
                    response = codeload_archive(intel_files, self.dependency_sha)
                elif url.startswith(prefix):
                    _, path = url[len(prefix):].split('/', 1)
                    response = intel_files[path]
        ...
        with tempfile.TemporaryDirectory() as directory, patch.object(Run, 'unit', count_unit):
            ...
            # A different immutable commit with the identical manifest/storage
            # inventory contains no Intel facts to rescan.
            ...
            self.assertIsNone(state['continuation'])
            self.assertEqual(state['revision'], OLDER)
            self.assertEqual(state['completed_watermark'], '2026-09-09T05:00:00Z')
