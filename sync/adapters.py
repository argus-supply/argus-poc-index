...
class Run:
    ...
    def intel(self):
        ...
        if dep.get('mode') == 'full':
            self.cursor['intel_full_snapshot'] = True
        elif dep.get('base_commit_sha'):
            if self.cursor.get('intel_base_commit_sha') not in (None, dep['base_commit_sha']):
                raise ValueError('resume requires the original intel dependency base')
            self.cursor['intel_base_commit_sha'] = dep['base_commit_sha']
    ...
    def poc(self):
        ...
        if (not self.old.get('continuation') and self.old.get('status') == 'ok' and
            ...
            self.finish(revision, watermark=self.old.get('completed_watermark'))
            return
        active = self.dependency.get('active_ids')
        if not isinstance(active, list) or any(not isinstance(a, str) or not CVE.fullmatch(a) for a in active):
        ...
        if 'previous_active_ids' in self.dependency:
            self.state.pop('active_ids', None)
        else:
    ...
    def poc_plan(self, active, revision):
        ...
        if self.old.get('continuation'):
            ...
            return False
        previous_active = self.dependency.get('previous_active_ids')
    ...
    def official(self):
        ...
        for index in range(self.cursor.get('offset', 0), len(intel)):
            ...
            for reference_index, ref in enumerate(advisory.get('references', [])):
                ...
                self.add(row)
            self.cursor['offset'] = index + 1
        if self.dependency.get('mode') == 'incremental':
            affected = ({row['record_id'] for row in intel}
                        | set(self.dependency.get('deleted_record_ids', [])))
            desired = {row['record_id'] for row in self.result.records}
            for row in self.previous:
                source_ids = {item.get('source_record_id') for item in row.get('provenance', [])
                              if isinstance(item, dict)}
                if (source_ids & affected and row.get('intel_commit_sha') == intel_sha
                        and row.get('status') == 'active'):
                    desired.add(row['record_id'])
            for row in self.previous:
                source_ids = {item.get('source_record_id') for item in row.get('provenance', [])
                              if isinstance(item, dict)}
                if (source_ids & affected and row['record_id'] not in desired
                        and row.get('status') != 'source_deleted'):
                    removed = copy.deepcopy(row)
                    removed.update(status='source_deleted', source_modified_at=self.now,
                                   intel_commit_sha=intel_sha)
                    self.add(removed)
