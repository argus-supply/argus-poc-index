...
def run(repository, remote, work, job_id, *, policy, token=None, manual=False, http=None):
    ...
    dependency_files, dependency_state = {}, None
    changed_bytes = 0
    ...
    try:
        if repository == 'argus-poc-index':
            ...
            bases = {state.get('continuation', {}).get('intel_base_commit_sha') for state in sources.values()
                     if isinstance(state.get('continuation'), dict)} - {None}
            if len(bases) > 1:
                raise ValueError('conflicting intel continuation bases')
            force_full = any(state.get('continuation', {}).get('intel_full_snapshot') is True
                             for state in sources.values() if isinstance(state.get('continuation'), dict))
            dependency, dependency_state, dependency_files = consume_intel(
                client, next(iter(requested), None), previous, before_files,
                base_sha=next(iter(bases), None), force_full=force_full)
            dependencies = [{k: dependency[k] for k in ('repository', 'commit_sha', 'manifest_sha256')}]
            coverage_keys = ('repository', 'commit_sha', 'manifest_sha256', 'coverage_version',
                             'coverage_complete', 'required_sources', 'coverage_errors')
            dependency_coverage = {dependency['repository']:
                                   {key: dependency[key] for key in coverage_keys}}
            ...
            metrics['dependency_current_bytes'] = len(canonical(dependency_state))
        ...
        extra = {'intel_dependency_state': dependency_state} if dependency_state else {}
        ...
        for attempt in range(3):
            files, manifest = build_snapshot('argus-supply/' + repository, records, events, sources,
                ...
                extra, previous_files=before_files, run_id=job_id)
